...
| Server | Express + Node.js 20 + TypeScript | Single service: API + worker pipeline. Deployed on Railway/Fly.io |
...
│   └── types.ts                    # SDK types
├── server/                         # Express server: API + worker pipeline (deploys to Railway/Fly.io)
│   ├── prisma/
...
- Node.js 20+, pnpm 9+
- Docker (for Fly.io deploys, optional)
...
- Railway or Fly.io account → for server deployment
...
NEXT_PUBLIC_API_URL=       # Server URL (e.g., https://guardrails-server.railway.app)
```
...
- **Sun:** Deploy server to Railway/Fly.io. Load test with 100 txn/min synthetic stream. Tune pre-filter ratios.
